...
func TestUnmarshalErrors(t *testing.T) {
	tests := []struct {
		...
		{[]byte{
			...
			0x0, // (wrong) string offsets start marker
		}, "could not read string offsets: unknown value type: 0"},
		{[]byte{
			0x1, 0x2, 0x3, 0x4, // format signature
			0x1, 0x0, 0x0, 0x0, // always 1
			0xc, 0x0, 0x0, 0x0, // string offsets start offset (12)
			0x1, // (wrong) string offsets start marker
		}, "read value is not a string offsets table"},
		{[]byte{
			0x1, 0x2, 0x3, 0x4, // format signature
			0x1, 0x0, 0x0, 0x0, // always 1
			0xd, 0x0, 0x0, 0x0, // string offsets start offset (13)
			0x1,                    // (wrong) dictionary type start marker
			0x7,                    // string offsets start marker
		...
		{[]byte{
			...
			0x0, 0x0, 0x0, 0x0, // string offsets index 0: "a"
	...
	for _, tt := range tests {
		_, err := ggdict.Unmarshal(tt.data, ggdict.FormatThimbleweed)
		if err == nil {
